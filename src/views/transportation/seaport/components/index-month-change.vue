...
<script setup>
  ...
  const getData = async () => {
    const res = await passengerAndFreightIndexMonthlyChange(selectRadio.value) || []
    date.value = res[0] && `${res[0].report_date}`
    newData.value = res.map(item => [`${item.year}-${item.month}`, item.sum_numerical])
  }
  ...
  watch(() => selectRadio.value, () => { getData() }, {
    deep: true,
    immediate: true
  })
</script>
