...
<script setup>
  ...
  import { historicalPassengerAndFreightIndexChange } from '@/api/modules/transportation/airport.js'
  ...
  const chartData = ref([])
  const getData = async () => {
    const res = await historicalPassengerAndFreightIndexChange()
    chartData.value = res
  }
  getData()
  ...
  watch(
    ...
    () => {
      ...
      newData.forEach((item, i) => {
        ...
        if (fd) {
          fd.data.push(item.sum_numerical)
        }
      })
