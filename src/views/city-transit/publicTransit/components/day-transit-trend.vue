...
<script setup>
  ...
  const getData = async () => {
    const res = await Api.dayOfMonthTrend()
    const dataObj = {}
    res.forEach(item => {
      dataObj[item.category] = dataObj[item.category] || []
      dataObj[item.category].push(item)
    })
    config.forEach(item => {
      const fd = Object.keys(dataObj).find(o => item.name.includes(o)) || ''
      item.data = (dataObj[fd] || []).map(obj => [obj.report_date, obj.numerical])
    })
  }
        ...
        formatter (p) {
          ...
          param.forEach(item => {
            ...
            <div class="tip-data">
              ...
              <span}">：${formatNumStr(item.value[1])}万人次</span>
            </div>
          `
          })
          return `
