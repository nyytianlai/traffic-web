 ...
 * @LastEditTime: 2022-12-12 15:41:52
 * @FilePath: \traffic-web\src\views\city-transit\taxi\components\online-taxi.vue
...
<script setup>
  ...
  const option = computed(() => {
    ...
    const series = []
    // const unit = config.value[0] && config.value[0].unit
    config.value.forEach((item, i) => {
      const serie = {
        name: item.name,
        color: item.color,
        type: 'line',
  ...
  watch(() => [props.data, props.config], () => {
    nextTick(() => {
      config.value = props.config.map((item, i) => {
        const data = []
        props.data.forEach(obj => {
          if (obj.zbmc === item.otherName) {
            data.push([obj.sjwd, obj.zbz])
          }
        })
        return { ...item, data }
      })
