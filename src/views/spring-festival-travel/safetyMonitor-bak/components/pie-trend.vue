...
<script setup>
  ...
  const total = computed(() => {
    const fd = props.data.find(item => item.zbmc === `${props.type}总客运量`)
  ...
  watch(() => [props.data, props.type], () => {
    nextTick(() => {
      config.forEach(item => {
        const fd = props.data.find(obj => obj.zbmc === `${props.type}${item.dataName}`)
        const fd2 = props.data.find(obj => obj.zbmc === `${props.type}${item.dataName}占比`)
        item.value = fd?.zbz
        item.percentVl = fd2?.zbz
      })
    })
  }, {
    deep: true,
